...
class Reachability: NSObject, ReachabilityInterface {
    ...
    init?(hostAddress: sockaddr_in) {
        ...
        guard let defaultReachability = withUnsafePointer(to: &address, {
    ...
    static func networkReachabilityForInternetConnection() -> ReachabilityInterface? {
        var zeroAddress = sockaddr_in()
        zeroAddress.sin_len = UInt8(MemoryLayout.size(ofValue: zeroAddress))
        zeroAddress.sin_family = sa_family_t(AF_INET)
        return Reachability(hostAddress: zeroAddress) as ReachabilityInterface?
    }
    
    static func networkReachabilityForLocalWiFi() -> ReachabilityInterface? {
        ...
        return Reachability(hostAddress: localWifiAddress) as ReachabilityInterface?
    }
